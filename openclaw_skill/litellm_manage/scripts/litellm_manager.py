...
Usage:
    ...
    python litellm_manager.py key update --key "sk-xxx" --budget 30
    python litellm_manager.py key suspend --key "sk-xxx"
    python litellm_manager.py key activate --key "sk-xxx"
    python litellm_manager.py key quota-reset --key "sk-xxx"
    ...
    python litellm_manager.py user info --user-id "xxx"
    python litellm_manager.py user update --user-id "xxx" --budget 50
    python litellm_manager.py user delete --user-id "xxx"

    # Team Management
    python litellm_manager.py team list
    python litellm_manager.py team new --name "研发组" --budget 100
    python litellm_manager.py team info --team-id "xxx"
    python litellm_manager.py team update --team-id "xxx" --budget 200
    python litellm_manager.py team add-member --team-id "xxx" --user-id "yyy"
    python litellm_manager.py team remove-member --team-id "xxx" --user-id "yyy"
    python litellm_manager.py team delete --team-id "xxx"
    ...
    if new_key:
        ...
        if args.save:
            ...
            with open(save_path, "w") as f:
                json.dump(
                    {"api_key": new_key, "alias": args.alias, "budget": args.budget},
                    f,
                    indent=2,
                )
...
def cmd_key_delete(args):
    ...
    # Find key
    key_to_delete, _ = _find_key_by_input(key)
...
def _find_key_by_input(key_input: str) -> tuple:
    """Find actual key by partial match, alias, or full key.

    Returns:
        tuple: (actual_key, key_info_dict) or (None, None) if not found
    """
    # First try to get info directly assuming it's a full key
    result = api_call("GET", f"/key/info?key={key_input}")
    if "info" in result:
        return key_input, result["info"]

    # If not found, get all keys and try to match by alias or partial key
    result = api_call("GET", "/key/list", {"limit": 100})
    ...
    for k in keys:
        ...
        if k == key_input:
            info_result = api_call("GET", f"/key/info?key={k}")
            if "info" in info_result:
                return k, info_result["info"]
        # Check if input is a substring of key
        if isinstance(k, str) and key_input in k:
            info_result = api_call("GET", f"/key/info?key={k}")
            if "info" in info_result:
                return k, info_result["info"]
...
def cmd_key_info(args):
    ...
    # Find key and get info
    actual_key, info = _find_key_by_input(key)
    ...
    print(f"  Last Active: {info.get('last_active') or 'Never'}")
...
def cmd_key_regenerate(args):
    ...
    # Find key
    # Find key
    old_key, _ = _find_key_by_input(key)
...
def cmd_key_update(args):
    """Update API key settings."""
    key = args.key

    # Find key
    key_to_update, _ = _find_key_by_input(key)
    ...
    # Build update data
    data = {"key": key_to_update}

    if args.alias:
        data["key_alias"] = args.alias
    ...
    if len(data) == 1:
        print_status(
            False, "No fields to update. Use --alias, --budget, --models, etc."
        )
        return
    ...
    if "error" not in str(result).lower():
        print_status(True, f"Key updated: {key_to_update[:12]}...")
        if args.alias:
            print(f"  New alias: {args.alias}")
        if args.budget is not None:
            print(f"  New budget: ${args.budget}")
        if args.models:
            print(f"  Models: {args.models}")
    else:
        print_status(False, f"Update failed: {result}")


def cmd_key_suspend(args):
    """Suspend (disable) API key."""
    key = args.key

    # Find key
    key_to_suspend, _ = _find_key_by_input(key)

    if not key_to_suspend:
        print_status(False, f"Key not found: {key}")
        return

    # Suspend by setting budget to 0
    data = {"key": key_to_suspend, "max_budget": 0}

    result = api_call("POST", "/key/update", data)

    if "error" not in str(result).lower():
        print_status(True, f"Key suspended: {key_to_suspend[:12]}...")
    else:
        print_status(False, f"Suspend failed: {result}")


def cmd_key_activate(args):
    """Activate (re-enable) API key."""
    key = args.key

    # Find key and get current info
    key_to_activate, key_data = _find_key_by_input(key)

    if not key_to_activate:
        print_status(False, f"Key not found: {key}")
        return

    # Activate by restoring original budget or setting new one
    budget = args.budget if args.budget else key_data.get("max_budget", 20)
    data = {"key": key_to_activate, "max_budget": budget}

    result = api_call("POST", "/key/update", data)

    if "error" not in str(result).lower():
        print_status(True, f"Key activated: {key_to_activate[:12]}...")
        print(f"  Budget restored to: ${budget}")
    else:
        print_status(False, f"Activate failed: {result}")


def cmd_key_quota_reset(args):
    """Reset key spend/quota."""
    key = args.key

    # Find key
    key_to_reset, _ = _find_key_by_input(key)

    if not key_to_reset:
        print_status(False, f"Key not found: {key}")
        return

    # Reset spend by updating key with spend_reset=True
    data = {"key": key_to_reset, "spend": 0}

    result = api_call("POST", "/key/update", data)

    if "error" not in str(result).lower():
        print_status(True, f"Key quota reset: {key_to_reset[:12]}...")
        print(f"  Spend reset to $0")
    else:
        print_status(False, f"Reset failed: {result}")
...
def cmd_user_info(args):
    ...
    result = api_call("GET", "/user/info", {"user_id": user_id})

    if "error" in str(result).lower():
        print_status(False, f"User not found: {user_id}")
        return
    ...
    print(f"  Alias: {user_info.get('user_alias') or 'N/A'}")
    print(f"  Email: {user_info.get('user_email') or 'N/A'}")
    print(f"  Spend: ${user_info.get('spend', 0):.2f}")
    print(f"  Budget: ${user_info.get('max_budget', 0):.2f}")
    print(f"  Created: {user_info.get('created_at') or 'N/A'}")
    print(f"  Last Active: {user_info.get('last_active') or 'Never'}")

    # Show user keys if available
    keys = user_info.get("keys", [])
    if keys:
        print(f"\n{BLUE}User Keys ({len(keys)}):{RESET}")
        for k in keys[:5]:
            key_alias = k.get("key_alias") or "N/A"
            key_preview = (k.get("key") or k.get("token") or "N/A")[:12] + "..."
            print(f"  • {key_alias}: {key_preview}")
        if len(keys) > 5:
    ...
    if teams:
        print(f"\n{BLUE}Team Membership ({len(teams)}):{RESET}")
        for t in teams:
            team_id = t.get("team_id", "N/A")
            team_alias = t.get("team_alias") or "N/A"
            print(f"  • {team_alias} ({team_id})")
...
def cmd_user_update(args):
    ...
    if len(data) == 1:
        print_status(False, "No fields to update. Use --alias, --email, or --budget")
        return

    result = api_call("POST", "/user/update", data)

    if "error" not in str(result).lower():
        print_status(True, f"User updated: {user_id}")
        if args.alias:
            print(f"  Alias: {args.alias}")
        if args.email:
...
def cmd_team_info(args):
    ...
    result = api_call("GET", "/team/info", {"team_id": team_id})

    if "error" in str(result).lower():
        print_status(False, f"Team not found: {team_id}")
        return

    team_info = result.get("team_info", result)

    print(f"\n{BLUE}Team Info:{RESET}")
    print(f"  Team ID: {team_info.get('team_id', team_id)}")
    print(f"  Alias: {team_info.get('team_alias') or 'N/A'}")
    print(f"  Spend: ${team_info.get('team_spend', 0):.2f}")
    print(f"  Budget: ${team_info.get('team_max_budget', 0):.2f}")
    print(f"  Created: {team_info.get('created_at') or 'N/A'}")

    # Show team members
    members = team_info.get("team_members", [])
    if members:
        print(f"\n{BLUE}Team Members ({len(members)}):{RESET}")
        for m in members:
            user_id = m.get("user_id") or m.get("user_email") or "N/A"
            role = m.get("role", "member")
            print(f"  • {user_id} ({role})")
    else:
        print(f"\n{BLUE}Team Members:{RESET} None")

    # Show team keys if available
    keys = team_info.get("keys", [])
    if keys:
        print(f"\n{BLUE}Team Keys ({len(keys)}):{RESET}")
        for k in keys[:5]:
            key_alias = k.get("key_alias") or "N/A"
            key_preview = (k.get("key") or k.get("token") or "N/A")[:12] + "..."
            spend = k.get("spend", 0)
            print(f"  • {key_alias}: {key_preview} (spend: ${spend:.2f})")
        if len(keys) > 5:
            print(f"  ... and {len(keys) - 5} more")
...
def cmd_team_update(args):
    ...
    if args.alias:
        data["team_alias"] = args.alias
    if args.budget is not None:
        data["team_max_budget"] = args.budget

    if len(data) == 1:
        print_status(False, "No fields to update. Use --alias or --budget")
        return

    result = api_call("POST", "/team/update", data)

    if "error" not in str(result).lower():
        print_status(True, f"Team updated: {team_id}")
        if args.alias:
            print(f"  Alias: {args.alias}")
        if args.budget is not None:
            print(f"  Budget: ${args.budget}")
...
def cmd_team_add_member(args):
    ...
    data = {"team_id": team_id, "user_id": user_id, "role": args.role or "user"}

    result = api_call("POST", "/team/member_add", data)

    if "error" not in str(result).lower():
        print_status(True, f"Member added to team!")
        print(f"  Team: {team_id}")
        print(f"  User: {user_id}")
        print(f"  Role: {args.role or 'user'}")
    else:
        print_status(False, f"Failed: {result}")
...
def cmd_team_remove_member(args):
    ...
    result = api_call("POST", "/team/member_delete", data)

    if "error" not in str(result).lower():
        print_status(True, f"Member removed from team!")
        print(f"  Team: {team_id}")
        print(f"  User: {user_id}")
    else:
        print_status(False, f"Failed: {result}")
...
Examples:
  ...
  %(prog)s key update --key sk-xxx --budget 30
  %(prog)s key suspend --key sk-xxx
  %(prog)s key activate --key sk-xxx
  %(prog)s key quota-reset --key sk-xxx
  ...
  %(prog)s user info --user-id xxx
  %(prog)s user update --user-id xxx --budget 50
  %(prog)s user delete --user-id xxx

  # Team management
  %(prog)s team list
  %(prog)s team new --name "研发组" --budget 100
  %(prog)s team info --team-id xxx
  %(prog)s team update --team-id xxx --budget 200
  %(prog)s team add-member --team-id xxx --user-id yyy
  %(prog)s team remove-member --team-id xxx --user-id yyy
  %(prog)s team delete --team-id xxx
  ...
  %(prog)s logs --lines 100
    ...
    key_update = key_sub.add_parser("update", help="Update key settings")
    key_update.add_argument("--key", required=True, help="Key or alias")
    key_update.add_argument("--alias", help="New key alias")
    key_update.add_argument("--budget", type=float, help="New monthly budget")
    key_update.add_argument("--models", help="Comma-separated models")
    key_update.add_argument("--rpm", type=int, help="RPM limit")
    key_update.add_argument("--tpm", type=int, help="TPM limit")
    key_update.add_argument("--expires", help="Expiration date")

    key_suspend = key_sub.add_parser("suspend", help="Suspend (disable) key")
    key_suspend.add_argument("--key", required=True, help="Key or alias")

    key_activate = key_sub.add_parser("activate", help="Activate (re-enable) key")
    key_activate.add_argument("--key", required=True, help="Key or alias")
    key_activate.add_argument("--budget", type=float, help="Budget to restore")

    key_reset = key_sub.add_parser("quota-reset", help="Reset key spend/quota")
    key_reset.add_argument("--key", required=True, help="Key or alias")
    ...
    user_info.add_argument("--user-id", required=True, help="User ID")

    user_update = user_sub.add_parser("update", help="Update user settings")
    user_update.add_argument("--user-id", required=True, help="User ID")
    user_update.add_argument("--alias", help="New user alias")
    user_update.add_argument("--email", help="New email")
    user_update.add_argument("--budget", type=float, help="New budget")
    ...
    team_info = team_sub.add_parser("info", help="Get team info")
    team_info.add_argument("--team-id", required=True, help="Team ID")

    team_update = team_sub.add_parser("update", help="Update team settings")
    team_update.add_argument("--team-id", required=True, help="Team ID")
    team_update.add_argument("--alias", help="New team alias/name")
    team_update.add_argument("--budget", type=float, help="New team budget")

    team_add_member = team_sub.add_parser("add-member", help="Add member to team")
    team_add_member.add_argument("--team-id", required=True, help="Team ID")
    team_add_member.add_argument("--user-id", required=True, help="User ID to add")
    team_add_member.add_argument(
        "--role", default="user", help="Member role (default: user)"
    )

    team_remove_member = team_sub.add_parser(
        "remove-member", help="Remove member from team"
    )
    team_remove_member.add_argument("--team-id", required=True, help="Team ID")
    team_remove_member.add_argument(
        "--user-id", required=True, help="User ID to remove"
    )
    ...
    config_openai.add_argument(
        "--output", default="~/litellm_config.json", help="Output path"
    )
    ...
    try:
        if args.command == "status":
            ...
            elif args.key_command == "update":
                cmd_key_update(args)
            elif args.key_command == "suspend":
                cmd_key_suspend(args)
            elif args.key_command == "activate":
                cmd_key_activate(args)
            elif args.key_command == "quota-reset":
                cmd_key_quota_reset(args)
            ...
            if args.team_command == "list":
                ...
                cmd_team_update(args)
            elif args.team_command == "add-member":
                cmd_team_add_member(args)
            elif args.team_command == "remove-member":
                cmd_team_remove_member(args)
